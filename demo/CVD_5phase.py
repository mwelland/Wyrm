...
p0 = create_bubble( [.2*Lx, .2*Lx, .2*Lx], .2*Lx)
p1 = create_bubble( [.8*Lx, .8*Lx, .8*Lx], .2*Lx)
p2 = create_bubble( [.2*Lx, .8*Lx, .8*Lx], .2*Lx)
p_tot = as_vector([p0,p1,p2,0])
p_n = as_vector([0,1,2,3,4])
p = inner(p_n,p_tot)
...
writer = writer([ 'cmesh', 'phase'], field_names, [eval(f) for f in field_names], mesh, "output_5P/output_5P.pvd")
